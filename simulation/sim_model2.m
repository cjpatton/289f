...
tau_x = 1*ones(N,1); 
tau_x([1 2 5 8 10]) = 3; % select the reluctant agents
...
%     cnt_xh = min( tau_x, cnt_xh ); % such that the counter is bounded
    ...
    if ( (mod(iter_no,10) == 0) || (flag == 0) )
        ...
        % If the agent is still adapting, dont touch it...
        if cnt_xh(src_node) <= 1
            xo_model1(src_node) = x_model1(src_node); xh_model1(src_node) = tmp_avg;
        end
        if cnt_xh(dst_node) <= 1
            xo_model1(dst_node) = x_model1(dst_node); xh_model1(dst_node) = tmp_avg;
        end
    end
