...
iter_max = 1e3; % the max. no. of iterations 
...
tau_x = 1*ones(N,1); 
tau_x(1:3) = 10; % select the reluctant agents. WLOG, we always assume the reluctant agent are the first
...
for iter_no = 1 : iter_max
    ...
    % The two agents comm. only when they are like-minded
    if abs(x_gossip(src_node) - x_gossip(dst_node)) <= threshold
    ...
    if ( (mod(iter_no,10) == 0) || (flag == 0) )
        ...
        if abs(x_model1(src_node) - x_model1(dst_node)) <= threshold
            ...
            xh_model1(src_node) = x_model1(src_node); xh_model1(dst_node) = x_model1(dst_node);
        end
    end
